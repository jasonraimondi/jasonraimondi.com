<script lang="ts">
  import LazyEmbed from "./LazyEmbed.svelte";

  interface Props {
    user: string;
    id: string;
    file?: string;
  }

  let { user, id, file }: Props = $props();

  const gistUrl = $derived(
    `https://gist.github.com/${user}/${id}.js${file ? `?file=${encodeURIComponent(file)}` : ""}`,
  );

  const srcdoc = $derived(
    `
			<!DOCTYPE html>
			<html>
			<head>
				<base target="_blank">
				<style>
					body { margin: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
					.gist .gist-file { margin: 0 !important; border: none !important; }
					.gist .gist-data { border: none !important; }
				</style>
			</head>
			<body>
				<script src="${gistUrl}"><` +
      `/script>
			</body>
			</html>
		`,
  );
</script>

<LazyEmbed {srcdoc} title="GitHub Gist: {id}" minHeight={100}>
  {#snippet placeholder(loading)}
    <div class="gist-placeholder">
      <div class="gist-placeholder-icon">
        <svg viewBox="0 0 16 16" width="24" height="24" aria-hidden="true">
          <path
            fill="currentColor"
            d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z"
          ></path>
        </svg>
      </div>
      <span class="gist-placeholder-text">
        {#if loading}
          Loading gist...
        {:else}
          GitHub Gist
        {/if}
      </span>
    </div>
  {/snippet}
</LazyEmbed>

<style>
  .gist-placeholder {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    gap: 0.75rem;
    padding: 2rem;
    background: var(--color-gray-100);
    border: 1px solid var(--color-gray-200);
    border-radius: 6px;
    min-height: 100px;
  }

  :global([data-theme="dark"]) .gist-placeholder {
    background: var(--color-gray-800);
    border-color: var(--color-gray-700);
  }

  .gist-placeholder-icon {
    color: var(--color-gray-500);
  }

  :global([data-theme="dark"]) .gist-placeholder-icon {
    color: var(--color-gray-400);
  }

  .gist-placeholder-text {
    font-size: 0.875rem;
    color: var(--color-gray-600);
  }

  :global([data-theme="dark"]) .gist-placeholder-text {
    color: var(--color-gray-400);
  }
</style>
